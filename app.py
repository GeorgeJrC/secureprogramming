import os
import sqlite3
from flask import Flask, render_template, request, Response, redirect, url_for, flash, session, send_from_directory, abort, send_file,make_response,jsonify
...
def comments():
    if request.method == 'POST':
        ...
        # Sanitize inputs
        # Escape is used to prevent SQL Injection as it converts special characters to their corresponding HTML value
...
def login(): #secure login using parameterized query
    if request.method == 'POST':
        username = request.form['username']
